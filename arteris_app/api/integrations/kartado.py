...
class KartadoAPIClient:
    
    def create_kartado_measurement_records(start_date: datetime, end_date: datetime = datetime.now()):
        ...
        #def get_kartado_data(record_type: str):
                ...
                for record in records:
                    ...
                    if record["chave_contrato"] not in contracts:
                        ...
                        if data["message"]:
                            ...
                            if not data["message"]["kartado"]:
                                # Write relationship to kartado
                                update_contract(
                                    contract=data["message"]["name"],
                                    kartado_uuid=record["chave_contrato"]
                                )

                            # Update contract information
                            contract["name"] = data["message"]["name"]
                            contract["kartado"] = record["chave_contrato"]
                            contract["code"] = record["contrato"] 

                            # Get contract items
                            data = get_contract_items(
                                contract_name=contract["name"]
                            )
                            itens = data["message"]
    ...
    #     #create_kartado_measurement_records(datetime.combine(date(2025, 2, 18), datetime.min.time()),datetime.combine(date(2025, 2, 24), datetime.min.time()))
    #     create_kartado_measurement_records(datetime.combine(date(2025, 5, 8), datetime.min.time()), datetime.combine(date(2025, 6, 1), datetime.min.time()))